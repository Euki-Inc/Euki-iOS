...
class CycleManager: NSObject {
	...
	private func generateCycleItems(dict: [Date: CalendarItem], responseHandler: @escaping ([CycleDayItem]) -> Void) {
        ...
        CalendarManager.sharedInstance.predictionRange { [weak self] predictionRanges in
            ...
            //workaround to have the same dayCicle in both summaries
            if let last = items.last, last.isToday(){
	...
	func convert(calendarItems: [CalendarItem]) -> CyclePeriodData {
        ...
        if let currentDay = currentDay{
            currentDayCycle = currentDay.dayCycle
        }
